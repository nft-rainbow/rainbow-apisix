...
func DefaultRequestFilter(o Parser, w http.ResponseWriter, r pkgHTTP.Request) (ReqParseResult, error) {
	...
	log.Infof("[%s] start parse request: path %s, method %s", r.ID(), r.Path(), r.Method())
	result, err := fn()
	if err != nil {
		log.Infof("[%s] failed parse request: path %s, method %s, err %s", r.ID(), r.Path(), r.Method(), err.Error())
		w.WriteHeader(http.StatusBadRequest)
		if _, err := w.Write([]byte(fmt.Sprintf("failed parse request: %v", err))); err != nil {
			log.Errorf("failed to write: %s", err)
		}
	}
	log.Infof("[%s] parse request result: path %s, method %s, cost-type %s, count %v", r.ID(), r.Path(), r.Method(), result.GetCostType(), result.GetCount())
	return result, err
}
